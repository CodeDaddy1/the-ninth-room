...
def main() -> int:
    ...
    if not transcript_segments and not args.no_whisper and video_path and meta.get("has_audio"):
        # LOCAL PATCH (curated-curiosities): before reaching for a paid Whisper
        # API, try the faster-whisper install this machine already has — same
        # engine the editing pipeline uses, $0, no key. Falls through to the
        # API path untouched if the import fails.
        try:
            from faster_whisper import WhisperModel
            print("[watch] transcribing locally with faster-whisper base.en", file=sys.stderr)
            model = WhisperModel("base.en", device="cpu", compute_type="int8")
            segments_iter, _info = model.transcribe(str(video_path))
            transcript_segments = [
